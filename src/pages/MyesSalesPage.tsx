...
export function MyesSalesPage() {
  const [loading, setLoading] = useState(true);
  const [rows, setRows] = useState<any[]>([]);
  const [status, setStatus] = useState<'all'|'pending'|'invoice_issued'|'payment_pending'|'paid'|'cancelled'>('all');
  ...
  const load = async () => {
    ...
    const { data } = await supabase
      .from('sales_requests')
      .select('id, status, created_at, property:properties(title,address), buyer:profiles(full_name), realtor:profiles(full_name)')
  ...
  const view = useMemo(()=>{
    ...
    const arr = rows
      ...
      .filter(r => !qq || (r.id||'').toLowerCase().includes(qq) || (r.property?.title||'').toLowerCase().includes(qq) || (r.property?.address||'').toLowerCase().includes(qq));
      ...
      <div className="p-4 border-b flex flex-wrap items-center gap-2 justify-between">
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-2 border rounded-xl px-3 py-1.5">
            ...
            <input value={q} onChange={(e)=>setQ(e.target.value)} placeholder="Поиск по ID/объекту" className="bg-transparent outline-none" />
          </div>
          <div className="flex items-center gap-2 border rounded-xl px-3 py-1.5">
          ...
          <div className="flex items-center gap-1">
            {(['all','pending','invoice_issued','payment_pending','paid','cancelled'] as const).map(s=>(
        ...
        {loading ? (<div className="p-6 text-gray-600">Загрузка…</div>) : view.length===0 ? (<div className="p-6 text-gray-600">Ничего не найдено</div>) : view.map(r => (
          <div key={r.id} className="p-3 text-sm flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="font-medium text-gray-900 truncate">{r.property?.title || 'Объект'} • <span className="text-xs font-normal text-gray-600">{r.status}</span></div>
