...
export function LovePayLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (prof?.role !== 'lovepay') {
        ...
        throw new Error('Доступ только для сотрудников Love&Pay');
      }
      navigate('/lovepay/requests');
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <div className="flex items-center gap-2 mb-4 text-blue-700"><Shield className="w-5 h-5" /><span className="font-semibold">Love&Pay — вход</span></div>
        <form onSubmit={handleLogin} className="space-y-3">
          ...
          <button type="submit" disabled={loading} className="w-full px-4 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-600 text-white flex items-center justify-center gap-2">
